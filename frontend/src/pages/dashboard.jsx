    ...
    <div className="app-shell">
      ...
      <div className="main-content">
        ...
        <div style={{ padding: '36px 36px 0', marginBottom: 32 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div className="fade-up">
              <h1 className="text-3xl font-bold" style={{ marginBottom: 6 }}>{greeting}</h1>
              <p className="text-base" style={{ color: '#8B949E' }}>{subtext}</p>
            </div>
        ...
        {!skillProfile ? (
          <div style={{ padding: '0 36px', marginBottom: 28 }}>
            <div style={{ background: 'linear-gradient(135deg, rgba(240,165,0,0.08), rgba(240,165,0,0.03))', border: '1px solid rgba(240,165,0,0.2)', borderRadius: 20, padding: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <button className="btn btn-primary hover-lift" onClick={() => navigate('/assessment')} style={{ whiteSpace: 'nowrap' }}>
                Start Now →
              </button>
            </div>
          </div>
        ...
        <div className="dashboard-grid">
          {features.map((f, i) => (
            <div key={f.id} className={`feature-card fade-up-${i % 3} hover-lift`} onClick={() => navigate(f.path)} style={{ cursor: 'pointer' }}>
