          ...
          <div style={{ display: 'flex', background: '#0D1117', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: 4, marginBottom: 32 }}>
            {['register', 'login'].map(m => (
              <button key={m} onClick={() => setMode(m)}
                style={{ flex: 1, padding: '9px', border: 'none', borderRadius: 9, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontWeight: 600, fontSize: 13, transition: 'all 0.2s', background: mode === m ? '#F0A500' : 'transparent', color: mode === m ? '#000' : '#8B949E' }}>
