            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {[['all', 'All Jobs'], ['Government', '🏛️ Government'], ['Private', '🏢 Private']].map(([val, label]) => (
                <button key={val} onClick={() => setFilter(val)}
                  className={`btn btn-sm hover-scale ${filter === val ? 'btn-primary' : 'btn-ghost'}`}>
                  {label}
                </button>
              ))}
            </div>
        ...
        <div style={{ padding: '0 36px 36px' }}>
          ...
          {profile.recommended_path && (
            <div style={{ background: 'rgba(108,142,245,0.06)', border: '1px solid rgba(108,142,245,0.15)', borderRadius: 14, padding: '14px 18px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <button className="btn btn-ghost btn-sm hover-scale" style={{ marginLeft: 'auto' }} onClick={() => navigate('/assessment')}>Retake Assessment →</button>
            </div>
          )}

          {filtered.map((job, i) => (
            <div key={job.jobId} className={`job-card hover-lift ${selected === i ? 'expanded' : ''}`} style={{ marginBottom: 12 }}
              ...
              {selected === i && (
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <div style={{ display: 'flex', gap: 10 }}>
                    <button className="btn btn-primary btn-sm hover-glow">✋ Express Interest</button>
                    <button className="btn btn-ghost btn-sm hover-scale" onClick={e => { e.stopPropagation(); navigate('/passport') }}>🪪 View My Passport</button>
                  </div>
                </div>
              )}
            </div>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 20 }}>
            <button className="btn btn-primary hover-glow" onClick={() => navigate('/passport')} style={{ flex: 1, padding: 14 }}>🪪 Build Skill Passport</button>
            <button className="btn btn-ghost hover-scale" onClick={() => navigate('/roadmap')} style={{ flex: 1, padding: 14 }}>📚 Back to Roadmap</button>
          </div>
        </div>
      </div>
    </div>
